...
const getAstDiff = (beforeObj, afterObj) => {
  ...
  const reducer = (acc, key) => {
    if (_.has(beforeObj, key) && !_.has(afterObj, key)) {
      return [...acc, { nameProperty: key, statusProperty: 'deleted', oldValue: beforeObj[key] }];
    }
    if (!_.has(beforeObj, key) && _.has(afterObj, key)) {
      return [...acc, { nameProperty: key, statusProperty: 'added', newValue: afterObj[key] }];
    }
    if (_.isPlainObject(beforeObj[key]) && _.isPlainObject(afterObj[key])) {
      return [...acc, {
        nameProperty: key, statusProperty: 'nested', children: getAstDiff(beforeObj[key], afterObj[key], []),
      }];
    }
    if (beforeObj[key] === afterObj[key]) {
      return [...acc, { nameProperty: key, statusProperty: 'unchanged', value: beforeObj[key] }];
    }
    return [...acc, {
